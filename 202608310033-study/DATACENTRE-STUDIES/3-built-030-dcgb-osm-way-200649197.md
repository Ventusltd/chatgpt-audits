# DCGB-OSM-WAY-200649197

**Verdict: BUILT / UNKNOWN STAGE**  |  demand score 30 / 100  |  OSM element `DCGB-OSM-WAY-200649197`
...
| field | value |
|---|---|
| source record id | `DCGB-OSM-WAY-200649197` |
| OSM element | https://www.openstreetmap.org/way/200649197 |
| coordinates (lat, lon) | 53.0789763, -0.7920692 |
...
The nearest named point in the pinned V8 `datacentres` layer is 32.40 km away, recorded as "Experian".
...
| measure | value |
|---|---|
| nearest transmission circuit | 2.95 km at 400 kV |
| nearest substation | 0.45 km |
| that substation, voltage | 33000 / 11000 |
| that substation, name | _unnamed in source_ |

Within 2.95 km of a 400 kV circuit - a workable connection distance for a load of this kind.

## 4. Generation adjacency - the pairing that matters

| measure | value |
|---|---|
| nearest live solar or BESS project | 4.00 km |
| that project | Main Street, Kelham - Solar Farm & Battery Energy Storage |
| its capacity | 50 MW |
| its inferred window state | **CONSENTED_NO_DATE** |
| its REPD reference | `11998` |

A live generation project is close, but it is not in the commercial window (CONSENTED_NO_DATE). Note the adjacency and
